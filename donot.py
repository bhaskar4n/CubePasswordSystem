...
for i in range(len(vec1)):
    ...
    vx = vec1[s][i][4]*149597870.691*86400
    vy = vec1[s][i][5]*149597870.691*86400
    vz = vec1[s][i][6]*149597870.691*86400
...
def display_click_data(value):
    ...
    vy = chvy[value]
    vz = chvz[value]
    print('cx:',cx,'cy:',cy,'cz:',cz,'vx',vx,'vy',vy,'vz',vz)
    cr = math.sqrt(float(cx)*float(cx) + float(cy)*float(cy)+ float(cz)*float(cz))
    vl = math.sqrt(float(vx)*float(vx) + float(vy)*float(vy)+ float(vz)*float(vz))
    print ('cr:',cr,'vl',vl)
    lx = mx[int(value)+3318]
    ly = my[int(value)+3318]
    lz = mz[int(value)+3318]
    vx = mvx[int(value)+3318]
    vy = mvy[int(value)+3318]
    vz = mvz[int(value)+3318]
    ax = cx - lx
    ay = cy - ly
    az = cz - lz
    print('ax:',ax,'ay:',ay,'az:',az,'vx',vx,'vy',vy,'vz',vz)
    mr = math.sqrt(float(ax)*float(ax) + float(ay)*float(ay) + float(az)*float(az))
    mvl = math.sqrt(float(vx)*float(vx) + float(vy)*float(vy) + float(vz)*float(vz))
    print ('mr:',mr,'mvl',mvl)
    return str(dat)+'\n'+'chandrayaan \n'+'Relative distance btw earth & ch2: '+str(cr)+' km\n'+ 'Relative distance btw moon & ch2: '+str(mr)+' km\n'+'velocity wrt Earth'+str(vl)+'\n velocity wrt to moon:'+str(mvl)+'\n'
